        ...
        <div className="max-w-4xl mx-auto">
          ...
          {media.length > 0 && (
            <div className="bg-white p-4 md:p-8 rounded-lg shadow-md mt-8">
              <h3 className="text-xl font-semibold text-gray-800 mb-6">Uploaded Media</h3>
              
              {/* Calculate items to show (2 rows) */}
              {(() => {
                const itemsPerRow = 4; // lg:grid-cols-4
                const maxRows = 2;
                const maxItemsToShow = itemsPerRow * maxRows;
                const itemsToShow = media.slice(0, maxItemsToShow);
                const hasMoreItems = media.length > maxItemsToShow;
                
                return (
                  <>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {itemsToShow.map((item: MediaItem) => (
                    ...
                    {hasMoreItems && (
                      <div className="text-center mt-6">
                        ...
                        >
                          See More ({media.length - maxItemsToShow} more photos & videos)
                        </a>
                      </div>
                    )}
                  </>
          ...
          {media.length === 0 && (
            <div className="bg-white p-4 md:p-8 rounded-lg shadow-md mt-8">
              <h3 className="text-xl font-semibold text-gray-800 mb-4">Uploaded Media</h3>
              <div className="text-center text-gray-500 py-8">
                <p>No media uploaded yet. Be the first to share your memories!</p>
              </div>
            </div>
          )}
        </div>
